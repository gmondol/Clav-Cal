    ...
    >
      ...
      {(note.description || (note.collabProfiles && note.collabProfiles.some((p) => p.igUrl || p.twitterUrl || p.tiktokUrl))) && (
        <div className="text-[11px] text-zinc-500 mb-2 space-y-1">
          ...
          {note.collabProfiles && (() => {
            const igFollowers = note.collabProfiles.map((p) => p.igFollowers).filter(Boolean).join(' / ');
            const ttFollowers = note.collabProfiles.map((p) => p.tiktokFollowers).filter(Boolean).join(' / ');
            const xFollowers = note.collabProfiles.map((p) => p.twitterFollowers).filter(Boolean).join(' / ');
            ...
            return (
              <div className="flex gap-1.5 flex-wrap">
                {hasIg && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-pink-100 text-pink-600 flex items-center gap-1 font-medium">
                ...
                {hasTt && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-100 flex items-center gap-1 font-medium">
                    <svg width="9" height="9" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.5a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 0010.86 4.48 6.3 6.3 0 001.86-4.49V8.75a8.26 8.26 0 004.84 1.56V6.84a4.85 4.85 0 01-1.12-.15z"/></svg>
                    {ttFollowers || ''}
                  </span>
                )}
                {hasX && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-zinc-200 text-zinc-700 flex items-center gap-1 font-medium">
...
/* NoteEditor is now imported from @/components/ui/NoteEditor */
